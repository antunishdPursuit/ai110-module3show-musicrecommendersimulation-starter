...
This project implements a content-based music recommender for a catalog of 18
songs. It compares each song with a user's genre, mood, energy, tempo, valence,
danceability, and acousticness preferences, then returns five ranked results
with plain-language explanations. The project also tests realistic and
adversarial profiles to show how feature weights, missing genres, and conflicting
preferences affect the recommendations.
...
I tested three realistic profiles and three adversarial profiles. The realistic
profiles confirmed that matching genre, mood, and audio features produced
intuitive recommendations. The adversarial profiles exposed weaker behavior
when a requested genre was missing, preferences conflicted, or every numeric
target was set to the midpoint.

I also ran a sensitivity experiment on the average-user profile by removing the
genre and mood bonuses and ranking songs using only numeric features. In the
baseline run, **Coffee Shop Stories** ranked first with a score of 7.83. In the
numeric-only run, it fell to fifth with a score of 4.83, while **Slow Burn
Summer** became the top recommendation at 5.61. This showed that categorical
bonuses strongly influence the ranking when a user's numeric preferences are
not distinctive.
...
The recommender uses a small synthetic catalog, so it cannot represent the
variety found in a real music service. Genre and mood use exact matches, which
means related categories such as lofi and ambient receive no partial credit.
The system also treats each preference as one fixed target and cannot represent
someone who wants different music for studying, exercising, and relaxing.
Because the scoring weights are manually chosen, they can over-favor particular
genres or audio characteristics and create a narrow recommendation bubble.
...
Building this recommender showed me how raw song attributes can be transformed
into ranked predictions through a scoring recipe. It also showed me how
sensitive recommendations are to design choices: changing or removing only a
few bonuses can substantially reorder the results even when the song data stays
the same.

Bias can enter through the catalog, feature definitions, and manually selected
weights. A transparent explanation helps reveal why a song was selected, but
human judgment is still necessary to decide whether the available data and
ranking rules represent users fairly.
